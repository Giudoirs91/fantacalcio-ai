...
path = "data/raw/odds/Fiorentinanapoli.json"
...
print(f"=== QUOTE FIORENTINA - NAPOLI (Codice: {avv.get('codiceAvvenimento')}) ===")
print("Totale mercati:", len(info_agg))

targets = [
    "1X2", "U/O 2.5", "U/O 1.5", "GOAL/NOGOAL", "1X2 + U/O 2.5", "1X2 + U/O 1.5", "1X2 + U/O 3.5",
    "DC + MULTIGOAL 1-3", "DC + MULTIGOAL 2-4", "DC + MULTIGOAL 1-4", "DC + GOAL/NOGOAL",
...
for k, v in info_agg.items():
    ...
    for t in targets:
        if desc.upper() == t or (t in desc.upper() and len(desc) < 32):
            esiti = []
