...
squads_def = [
    ...
    },
    {
        ...
        'whyBeatsRivals': "<ul><li><b>Zero Rischio Turn-over</b>: Con Bonny, Pellegrino e Kean in rosa, non giochi mai in 10.</li><li><b>Pioggia di Bonus dai Rigori</b>: Con Calhanoglu, Gudmundsson e Kean hai i tiratori designati di 3 squadre.</li><li><b>Bilancio Perfetto</b>: 11 Titolari (873 CR) + 14 Riserve (91 CR) = 964 CR totali (36 CR liberi).</li></ul>"
    },
    {
        'id': 'squad_3412',
        ...
        'bench': {
            'P': ['Caprile', 'Sportiello'],
            'D': ['Zappa', 'Patric', 'Kolasinac', 'Pellegrini Lu.', 'Chalobah T.'],
            'C': ['Ferguson', 'Frendrup', 'Deiola', 'Zarraga'],
            'A': ['Bonny', 'Trepy', 'De Martis']
        },
...
for sq in squads_def:
    ...
    print(f"  TOTALE ROSA ({st_count + be_count} Calciatori): {tot} CR / 1000 CR (Residuo: {1000 - tot} CR)")
    assert tot <= 1000, f"Squad {sq['name']} exceeds 1000 CR!"
    assert st_count == 11, f"Squad {sq['name']} does not have 11 starters!"
    assert be_count == 14, f"Squad {sq['name']} does not have 14 bench players!"
    assert st_count + be_count == 25, f"Squad {sq['name']} does not have 25 players!"

js_content = f"""// ==============================================================================
// MODULO: 5 SQUADRE PERFETTE CONSIGLIATE DALL'AI (5 MODULI SU BUDGET 1000 CR)
// ==============================================================================
...
function renderAiSquadsTab() {{
    ...
    container.innerHTML = `
        <div class="ai-squads-header-panel">
            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:gap:14px;">
                <div>
                    <div style="display:flex;align-items:center;gap:10px;">
                        <span style="font-size:24px;">🧠</span>
                        <h2 style="margin:0;font-size:20px;font-weight:900;color:#fff;letter-spacing:-0.5px;">Master AI: Le 5 Rose Perfette da 1000 Crediti</h2>
                        <span style="background:rgba(0,242,254,0.15);border:1px solid var(--accent-cyan);color:var(--accent-cyan);font-size:11px;font-weight:800;padding:2px 8px;border-radius:6px;">LEGA A 8 - 25 SLOT RIGOROSI</span>
                    </div>
                    <p style="margin:4px 0 0 0;font-size:12.5px;color:var(--text-secondary);">
                        5 formazioni complete (11 Titolari + 14 Panchina = 25 Calciatori) con <b>costo totale garantito ≤ 1000 CR</b> per battere i tuoi 7 rivali (Davide, Alessandro, Enrico, Rosario, Michele, Ilario, Valerio).
                    </p>
                </div>
                ...
                <div class="ai-starter-lineup-card">
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
                        <div style="display:flex;align-items:center;gap:8px;">
                            ...
                            <b style="color:#fff;font-size:14px;">11 Titolari Tipo (${{currentSquad.formation}})</b>
                        </div>
            ...
            <div style="flex:0.9;min-width:320px;display:flex;flex-direction:column;gap:14px;">
                <div class="ai-bench-panel">
                    ...
                    <p style="font-size:11.5px;color:var(--text-muted);margin:0 0 10px 0;">
                        Tutti i 14 rincalzi sono acquistabili a 1-2 crediti o coperture dirette per rispettare rigorosamente il budget di 1000 CR.
                    </p>
...
with open('web/js/ai_squads.js', 'w', encoding='utf-8') as f:
    f.write(js_content)
print("\n--> FILE web/js/ai_squads.js AGGIORNATO CON SUCCESSO!")
