...
# Make sure CALENDAR_DATA is loaded at top of script
top_cal_load = '''
...
new_generator_code = '''
# ==============================================================================
# EXACT 1:1 PLAYER PROFILE RENDERER (IDENTICAL TO IN-APP MODAL)
# ==============================================================================
...
def generate_player_page(player, injuries_db, tactical_db):
    ...
    is_in_11 = player.get("is_in_11", titolarita >= 75)
    ...
    # Expected FantaMedia (xFM) & Regression
    ...
    # Voti dettaglio 2627 & Smart Trend
    ...
    if latest_match:
        cal_info = get_match_info_for_team_and_round(team, latest_match.get("giornata", 1))
        ...
        next_m = get_match_info_for_team_and_round(team, 1)
        initial_inspector_html = f'''
        <div class="match-inspector-card" id="seasonMatchInspectorCard">
            <div class="inspector-header">
                <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
    ...
    if voti_list:
        for v in voti_list:
            ...
            cal_info = get_match_info_for_team_and_round(team, v.get("giornata", 1))
...
# Find the start of generate_player_page and end of it (before generate_injuries_pillar)
idx_start = orig_code.find("def generate_player_page(")
idx_end = orig_code.find("def generate_injuries_pillar(")

if idx_start != -1 and idx_end != -1:
    new_code = orig_code[:idx_start] + new_generator_code + "\n" + orig_code[idx_end:]
    
    # Also ensure CALENDAR_DATA is loaded at top
    if "CALENDAR_DATA" not in new_code:
        new_code = top_cal_load + "\n" + new_code
        
    with open(script_path, "w", encoding="utf-8") as f:
        f.write(new_code)
    print("SUCCESS: generate_player_page replaced with 1:1 identical in-app modal architecture!")
else:
