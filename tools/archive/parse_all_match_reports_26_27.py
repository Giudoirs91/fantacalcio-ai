...
def extract_match_info(pdf, filename):
    giornata = 1
...
def extract_formations(pdf):
    p2 = pdf.pages[1] if len(pdf.pages) > 1 else None
    if not p2:
        return {}
    txt = p2.extract_text() or ""
    
    # Formations like 4-3-3, 3-5-2, 4-2-3-1, 3-4-2-1
    moduli = re.findall(r'\b(\d-\d-\d(?:-\d)?)\b', txt)
    return moduli
...
def parse_player_stats(pdf, match_info):
    ...
    if len(tables) >= 3:
        ...
        team_tables = [
            (match_info['home_team'], match_info['away_team'], True, tables[0]),
            (match_info['away_team'], match_info['home_team'], False, tables[1])
        ]
...
def parse_all_reports():
    ...
    print(f"-> Trovati {len(pdf_files)} Match Report in '{REPORT_DIR}' (Giornate 1 e 2).")
    ...
    for filename in pdf_files:
        ...
        try:
            with pdfplumber.open(pdf_path) as pdf:
                match_info = extract_match_info(pdf, filename)
                moduli = extract_formations(pdf)
                match_info['moduli'] = moduli
                all_matches.append(match_info)
                
                p_stats = parse_player_stats(pdf, match_info)
                all_player_stats.extend(p_stats)
        except Exception as e:
            print(f"  [ERRORE] Impossibile parsare {filename}: {e}")
            
    df_players = pd.DataFrame(all_player_stats)
    df_players.to_csv(os.path.join(OUT_DIR, "match_reports_players_g1_g2.csv"), index=False)
    
    with open(os.path.join(OUT_DIR, "match_reports_matches_g1_g2.json"), "w", encoding="utf-8") as f:
        json.dump(all_matches, f, ensure_ascii=False, indent=2)
        
    print(f"\n✓ Parsing completato! Estratte {len(all_player_stats)} righe prestazione da {len(all_matches)} partite.")
    return df_players, all_matches
